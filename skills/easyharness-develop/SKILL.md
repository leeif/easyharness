...
3. **Read sdd-spec.md**: Extract per-task test scenarios (Given/When/Then). These define the concrete verification cases the evaluator will check.
4. **Read learnings**: If `docs/learnings.md` exists, read it. These are durable cross-task learnings from previous sessions.
5. **Create TodoWrite**: One todo per task from the plan. Pre-mark DONE tasks as `completed`.
6. **Verify dependencies**: Ensure test runner, linter, and build tools exist and are functional.
7. **Record baseline**: Run existing tests (count + status) and check lint status.
8. **Session recovery**: If resuming (some tasks marked DONE in plan.md), verify DONE tasks still pass (quick smoke test), then continue from first incomplete task. Learnings from prior sessions are already on disk — no context loss.
...
- Read task's SDD scenarios from `sdd-spec.md` — these define the exact test cases to implement.
...
- **Evaluator gets**: Task description, contract, SDD spec scenarios for this task, list of changed files, and current retry count.
- **Evaluator returns**: Structured JSON verdict (including SDD scenario compliance).
...
- **Input**: `plan.md` + `contract.md` + `sdd-spec.md` (typically generated by `easyharness-plan`).
