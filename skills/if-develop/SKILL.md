...
- Required: if-evaluator (dispatched as read-only subagent)
...
Dispatch `if-evaluator` as a read-only subagent. Use the `evaluator-dispatch-prompt.md` template.
...
## Integration
- **Required skills**: `if-evaluator`, `superpowers:test-driven-development`, `superpowers:verification-before-completion`.
- **Input**: `plan.md` + `contract.md` (typically generated by `if-plan`).
...
| EVALUATE | Dispatch if-evaluator | JSON verdict |
